# Supabase migration: objectives, logs, and scaling

This doc describes how to migrate Life Quests to Supabase while keeping objective logic, progress display, and the logbook scalable when users have hundreds of logs and many campaigns (e.g. 20–30). Use it when implementing the Supabase backend and adjusting the client.
...
- **Objective / period logic:** Changes to how we decide “achieved objective,” period boundaries (day/week/month), or where that logic lives (e.g. `date.ts`, `activity-stats.tsx`, `recent-logs.tsx`).
...
- **Objectives:** “Is this period’s goal met?” and “Did this log achieve the objective?” are computed in the client:
  ...
  - **Logbook** (`recent-logs.tsx`): For each displayed campaign log, `didLogAchieveObjective(log, activity, logs)` determines if that log was the one that pushed period progress to or over the goal; if so, we show “achieved objective” in green.
...
2. **Avoid recalculating “achieved objective”** for every log on every render; store or compute it in the DB and have the client display it.
...
- **Logbook:** Query “recent N logs” (e.g. `order by submitted_at desc limit 50`) and only fetch the columns needed for the list (including any “achieved objective” flag). The client never loads the full log history just to render the list.
...
## 2. Store or compute “achieved objective” in the DB

Today the client decides “did this log achieve the objective?” by scanning all logs in the period. With Supabase, do one of the following so the client only reads a result:

- **Option A – Stored flag:** When a log is inserted (or updated), run the “did this log achieve the objective?” logic once (in a DB function, trigger, or API) and set e.g. `achieved_period_objective boolean` on that row. The client then just reads that column for display (e.g. green “achieved objective” in the logbook).
- **Option B – Computed on read:** Expose an RPC (or view) that, for a given set of log ids (e.g. the 50 you’re about to show), returns which of them achieved their period objective. The client sends “these 50 log ids” and gets back a set of ids; the UI only needs to show the badge for those ids.

Either way, the rule “which log achieved the objective?” lives in one place (DB or backend), and the client only displays it.

**Objective rule to implement (server-side):** For a given log and its activity’s first goal (`amount`, `unit` = hours | occurrences, `timeRange` = day | week | month | year):
...
5. This log **achieved the objective** iff `progress_before < goal.amount` and `progress_including >= goal.amount`.
...
- [ ] **Achieved objective:** Either add `achieved_period_objective` (or equivalent) to the logs table and set it on insert/update, or add an RPC that returns which of a set of log ids achieved their objective; then update `recent-logs.tsx` to read and display that instead of calling `didLogAchieveObjective` in the client.
...
- [ ] **Optional:** Add memoization or smaller payloads in the client for any remaining client-side objective/progress work until the above is fully moved to the server.
...
| Logbook: “achieved objective” per log | `src/app/components/recent-logs.tsx`: `didLogAchieveObjective` |
...
- **2026-02-23:** Removed “Date Achieved” / “Date Accomplished” from logs. `ActivityLog` no longer has a `date` field; `submittedAt` (ISO timestamp) is required and is the single source for all date display and period/objective logic. Log creation no longer accepts a date—timestamp is set at submit time. Legacy logs loaded from storage are normalized via `normalizeLog()` so old `date`-only entries get a derived `submittedAt`. For Supabase, store only `submittedAt` (or equivalent) for log rows.
